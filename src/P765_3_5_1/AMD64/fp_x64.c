...
*
* Abstract: modular arithmetic optimized for x64 platforms for P765
*********************************************************************************************/
...
// Global constants
extern const uint64_t p765[NWORDS_FIELD];
extern const uint64_t p765p1[NWORDS_FIELD]; 
extern const uint64_t p765x2[NWORDS_FIELD]; 


__inline void fpadd765(const digit_t* a, const digit_t* b, digit_t* c)
{ // Modular addition, c = a+b mod p765.
  // Inputs: a, b in [0, 2*p765-1] 
  // Output: c in [0, 2*p765-1] 
    ...
    carry = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        SUBC(carry, c[i], ((digit_t*)p765x2)[i], carry, c[i]); 
    }
    mask = 0 - (digit_t)carry;

    carry = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(carry, c[i], ((digit_t*)p765x2)[i] & mask, carry, c[i]); 
    } 
...
__inline void fpsub765(const digit_t* a, const digit_t* b, digit_t* c)
{ // Modular subtraction, c = a-b mod p765.
  ...
  // Output: c in [0, 2*p765-1] 
    ...
    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(borrow, c[i], ((digit_t*)p765x2)[i] & mask, borrow, c[i]); 
    }
...
__inline void fpneg765(digit_t* a)
{ // Modular negation, a = -a mod p765.
  // Input/output: a in [0, 2*p765-1] 
    unsigned int i, borrow = 0;
    
    for (i = 0; i < NWORDS_FIELD; i++) {
        SUBC(borrow, ((digit_t*)p765x2)[i], a[i], borrow, a[i]); 
    }
}
...
{ // Modular division by two, c = a/2 mod p765.
  // Input : a in [0, 2*p765-1] 
  // Output: c in [0, 2*p765-1] 
    unsigned int i, carry = 0;
    digit_t mask;
        
    mask = 0 - (digit_t)(a[0] & 1);    // If a is odd compute a+p765
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(carry, a[i], ((digit_t*)p765)[i] & mask, carry, c[i]); 
    }
...
{ // Modular correction to reduce field element a in [0, 2*p765-1] to [0, p765-1].
    ...
    for (i = 0; i < NWORDS_FIELD; i++) {
        SUBC(borrow, a[i], ((digit_t*)p765)[i], borrow, a[i]); 
    }
    mask = 0 - (digit_t)borrow;

    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(borrow, a[i], ((digit_t*)p765)[i] & mask, borrow, a[i]); 
    }
}
...
{ // Efficient Montgomery reduction using comba and exploiting the special form of the prime p765.
  // mc = ma*R^-1 mod p765x2, where R = 2^768.
  // If ma < 2^768*p765, the output mc is in the range [0, 2*p765-1].
  // ma is assumed to be in Montgomery representation.
    ...
    mc[3] = ma[3];
    mc[4] = ma[4];
    MUL128(mc[0], ((digit_t*)p765p1)[5], uv);
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[6], uv, carry, uv);
    MULADD128(mc[1], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[1], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[1], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[1], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[1], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[0], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[1], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[1], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[2], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[2], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[3], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[3], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[4], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[4], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[5], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[5], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[6], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[5], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[6], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[7], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[6], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[7], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[8], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[7], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[8], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[9], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[8], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[9], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[10], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[9], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[10], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
    MULADD128(mc[11], ((digit_t*)p765p1)[10], uv, carry, uv);
    t += carry;
    ...
    MULADD128(mc[11], ((digit_t*)p765p1)[11], uv, carry, uv);
    t += carry;
