...
#define CRYPTO_SECRETKEYBYTES     384    // MSG_BYTES + SECRETKEY_B_BYTES + CRYPTO_PUBLICKEYBYTES bytes
...
// ----------------------------------------------------------------------
// Elements over GF(p434) are encoded in 94 octets in little endian format (i.e., the least significant octet is located in the lowest memory address). 
...
// Public keys pk consist of 3 elements in GF(p434^2). In the SIKE API, pk is encoded in 564 octets. 
...
// Output: the public key PublicKeyA consisting of 3 GF(p434^2) elements encoded in 564 bytes.
...
// The public key consists of 3 GF(p434^2) elements encoded in 564 bytes.
...
//         Bob's PublicKeyB consists of 3 GF(p434^2) elements encoded in 564 bytes.
// Output: a shared secret SharedSecretA that consists of one element in GF(p434^2) encoded in 188 bytes.
...
//         Alice's PublicKeyA consists of 3 GF(p434^2) elements encoded in 564 bytes.
// Output: a shared secret SharedSecretB that consists of one element in GF(p434^2) encoded in 188 bytes. 
...
// Public keys PublicKeyA and PublicKeyB consist of 3 elements in GF(p434^2). In the SIDH API, they are encoded in 564 octets. 
// Shared keys SharedSecretA and SharedSecretB consist of one element in GF(p434^2). In the SIDH API, they are encoded in 188 octets.
