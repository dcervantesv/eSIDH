...
{ // Tests for the field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing field arithmetic over GF(p443): \n\n"); 

    // Field addition over the prime p443
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom443_test(a); fprandom443_test(b); fprandom443_test(c); fprandom443_test(d); fprandom443_test(e); fprandom443_test(f); 

        fpadd443(a, b, d); fpadd443(d, c, e);                 // e = (a+b)+c
        fpadd443(b, c, d); fpadd443(d, a, f);                 // f = a+(b+c)
        fpcorrection443(e);
        fpcorrection443(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpadd443(a, b, d);                                     // d = a+b 
        fpadd443(b, a, e);                                     // e = b+a
        fpcorrection443(d);
        fpcorrection443(e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero443(b);
        fpadd443(a, b, d);                                     // d = a+0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero443(b);
        fpcopy443(a, d);     
        fpneg443(d);                      
        fpadd443(a, d, e);                                     // e = a+(-a)
        fpcorrection443(e);
    ...
    // Field subtraction over the prime p443
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom443_test(a); fprandom443_test(b); fprandom443_test(c); fprandom443_test(d); fprandom443_test(e); fprandom443_test(f); 

        fpsub443(a, b, d); fpsub443(d, c, e);                 // e = (a-b)-c
        fpadd443(b, c, d); fpsub443(a, d, f);                 // f = a-(b+c)
        fpcorrection443(e);
        fpcorrection443(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpsub443(a, b, d);                                     // d = a-b 
        fpsub443(b, a, e);                                         
        fpneg443(e);                                           // e = -(b-a)
        fpcorrection443(d);
        fpcorrection443(e);
        ...
        fpzero443(b);
        fpsub443(a, b, d);                                     // d = a-0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero443(b);
        fpcopy443(a, d);                 
        fpsub443(a, d, e);                                     // e = a+(-a)
        fpcorrection443(e);
        if (compare_words(e, b, NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Field multiplication over the prime p443
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fprandom443_test(a); fprandom443_test(b); fprandom443_test(c);  
        fprandom443_test(ma); fprandom443_test(mb); fprandom443_test(mc); fprandom443_test(md); fprandom443_test(me); fprandom443_test(mf); 
        ...
        fpmul443_mont(ma, mb, md); fpmul443_mont(md, mc, me);                          // e = (a*b)*c
        fpmul443_mont(mb, mc, md); fpmul443_mont(md, ma, mf);                          // f = a*(b*c)
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_mont(a, ma); to_mont(b, mb); to_mont(c, mc); 
        fpadd443(mb, mc, md); fpmul443_mont(ma, md, me);                               // e = a*(b+c)
        fpmul443_mont(ma, mb, md); fpmul443_mont(ma, mc, mf); fpadd443(md, mf, mf);    // f = a*b+a*c
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_mont(a, ma); to_mont(b, mb);
        fpmul443_mont(ma, mb, md);                                                      // d = a*b 
        fpmul443_mont(mb, ma, me);                                                      // e = b*a 
        from_mont(md, d);
        from_mont(me, e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_mont(a, ma);
        fpzero443(b); b[0] = 1; to_mont(b, mb);
        fpmul443_mont(ma, mb, md);                                                      // d = a*1  
        from_mont(ma, a);
        from_mont(md, d);                
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero443(b); to_mont(b, mb);
        fpmul443_mont(ma, mb, md);                                                      // d = a*0  
        from_mont(mb, b);
        from_mont(md, d);                
    ...
    // Field squaring over the prime p443
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom443_test(a);
        ...
        to_mont(a, ma);
        fpsqr443_mont(ma, mb);                                 // b = a^2
        fpmul443_mont(ma, ma, mc);                             // c = a*a 
        if (compare_words(mb, mc, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero443(a); to_mont(a, ma);
        fpsqr443_mont(ma, md);                                 // d = 0^2 
    ...
    // Field inversion over the prime p443
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom443_test(a); 
        to_mont(a, ma);
        fpzero443(d); d[0]=1; to_mont(d, md);
        fpcopy443(ma, mb);                            
        fpinv443_mont(ma);                                
        fpmul443_mont(ma, mb, mc);                             // c = a*a^-1 
...
{ // Tests for the quadratic extension field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing quadratic extension arithmetic over GF(p443^2): \n\n"); 

    // Addition over GF(p443^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random443_test((digit_t*)a); fp2random443_test((digit_t*)b); fp2random443_test((digit_t*)c); fp2random443_test((digit_t*)d); fp2random443_test((digit_t*)e); fp2random443_test((digit_t*)f); 

        fp2add443(a, b, d); fp2add443(d, c, e);                 // e = (a+b)+c
        fp2add443(b, c, d); fp2add443(d, a, f);                 // f = a+(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2add443(a, b, d);                                     // d = a+b 
        fp2add443(b, a, e);                                     // e = b+a
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero443(b);
        fp2add443(a, b, d);                                     // d = a+0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero443(b);
        fp2copy443(a, d);     
        fp2neg443(d);                      
        fp2add443(a, d, e);                                     // e = a+(-a)
    ...
    // Subtraction over GF(p443^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random443_test((digit_t*)a); fp2random443_test((digit_t*)b); fp2random443_test((digit_t*)c); fp2random443_test((digit_t*)d); fp2random443_test((digit_t*)e); fp2random443_test((digit_t*)f); 

        fp2sub443(a, b, d); fp2sub443(d, c, e);                 // e = (a-b)-c
        fp2add443(b, c, d); fp2sub443(a, d, f);                 // f = a-(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2sub443(a, b, d);                                     // d = a-b 
        fp2sub443(b, a, e);                                         
        fp2neg443(e);                                           // e = -(b-a)
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero443(b);
        fp2sub443(a, b, d);                                     // d = a-0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero443(b);
        fp2copy443(a, d);                 
        fp2sub443(a, d, e);                                     // e = a+(-a)
        if (compare_words((digit_t*)e, (digit_t*)b, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Multiplication over GF(p443^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    

        fp2random443_test((digit_t*)a); fp2random443_test((digit_t*)b); fp2random443_test((digit_t*)c);  
        fp2random443_test((digit_t*)ma); fp2random443_test((digit_t*)mb); fp2random443_test((digit_t*)mc); fp2random443_test((digit_t*)md); fp2random443_test((digit_t*)me); fp2random443_test((digit_t*)mf); 

        to_fp2mont(a, ma);
        fp2copy443(ma, mc);
        from_fp2mont(mc, c);
        if (compare_words((digit_t*)a, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; printf("uno"); break; }
        
        to_fp2mont(a, ma); to_fp2mont(b, mb); to_fp2mont(c, mc); 
        fp2mul443_mont(ma, mb, md); fp2mul443_mont(md, mc, me);                          // e = (a*b)*c
        fp2mul443_mont(mb, mc, md); fp2mul443_mont(md, ma, mf);                          // f = a*(b*c)
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; printf("dos"); break; }
      
        to_fp2mont(a, ma); to_fp2mont(b, mb); to_fp2mont(c, mc); 
        fp2add443(mb, mc, md); fp2mul443_mont(ma, md, me);                               // e = a*(b+c)
        fp2mul443_mont(ma, mb, md); fp2mul443_mont(ma, mc, mf); fp2add443(md, mf, mf);   // f = a*b+a*c
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; printf("tres"); break; }
       
        to_fp2mont(a, ma); to_fp2mont(b, mb);
        fp2mul443_mont(ma, mb, md);                                                      // d = a*b 
        fp2mul443_mont(mb, ma, me);                                                      // e = b*a 
        from_fp2mont(md, d);
        from_fp2mont(me, e);
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; printf("cuatro"); break; }

        
        to_fp2mont(a, ma);
        fp2zero443(b); b[0][0] = 1; 
        to_fp2mont(b, mb);
        fp2mul443_mont(ma, mb, md);                                                     // d = a*1  
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; printf("cinco"); break; }
        
        fp2zero443(b); to_fp2mont(b, mb);
        fp2mul443_mont(ma, mb, md);                                                      // d = a*0 
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)b, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; printf("seis"); break; } 
    }
    if (passed==1) printf("  GF(p^2) multiplication tests .................................... PASSED");
    else { printf("  GF(p^2) multiplication tests... FAILED"); printf("\n"); return false; }
    printf("\n");

    // Squaring over GF(p443^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random443_test((digit_t*)a);
        
        to_fp2mont(a, ma);
        fp2sqr443_mont(ma, mb);                                 // b = a^2
        fp2mul443_mont(ma, ma, mc);                             // c = a*a 
        from_fp2mont(mb, b);               
        from_fp2mont(mc, c);               
        if (compare_words((digit_t*)b, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero443(a); to_fp2mont(a, ma);
        fp2sqr443_mont(ma, md);                                 // d = 0^2 
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        to_fp2mont(a, ma);
        fp2zero443(d); d[0][0]=1; to_fp2mont(d, md);
        fp2copy443(ma, mb);                            
        fp2inv443_mont(ma);                                
        fp2mul443_mont(ma, mb, mc);                             // c = a*a^-1              
        from_fp2mont(mc, c);  
...
bool fp_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking field arithmetic over GF(p443): \n\n"); 
        
    fprandom443_test(a); fprandom443_test(b); fprandom443_test(c);

    // GF(p) addition using p443
    cycles = 0;
    ...
    // GF(p) multiplication using p443
    cycles = 0;
    for (n=0; n<BENCH_LOOPS; n++)
    {
        cycles1 = cpucycles(); 
...
bool fp2_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking quadratic extension arithmetic over GF(p443^2): \n\n"); 
...
bool ecisog_run()
{
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random443_test((digit_t*)A24); fp2random443_test((digit_t*)C24);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random443_test((digit_t*)A); fp2random443_test((digit_t*)coeff[0]); fp2random443_test((digit_t*)coeff[1]); fp2random443_test((digit_t*)coeff[2]);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random443_test((digit_t*)A4); fp2random443_test((digit_t*)C);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random443_test((digit_t*)A); fp2random443_test((digit_t*)C);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random443_test((digit_t*)A24m); fp2random443_test((digit_t*)A24p); fp2random443_test((digit_t*)C);
...
int main()
{
    ...
    OK = OK && fp_test();          // Test field operations using p443
    OK = OK && fp_run();           // Benchmark field operations using p443

    OK = OK && fp2_test();         // Test arithmetic functions over GF(p443^2)
    OK = OK && fp2_run();          // Benchmark arithmetic functions over GF(p443^2)
