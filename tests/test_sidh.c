...
{ // Testing key exchange
    ...
    for (i = 0; i < TEST_LOOPS; i++) 
    {
       ...
       random_mod_order_B(PrivateKeyB);
        ...
        EphemeralKeyGeneration_A((unsigned char*)PrivateKeyA, PublicKeyA);                            // Get some value as Alice's secret key and compute Alice's public key
        EphemeralKeyGeneration_B((unsigned char*)PrivateKeyB, PublicKeyB);                            // Get some value as Bob's secret key and compute Bob's public key
        EphemeralSecretAgreement_A((unsigned char*)PrivateKeyA, PublicKeyB, SharedSecretA);           // Alice computes her shared secret using Bob's public key
        EphemeralSecretAgreement_B((unsigned char*)PrivateKeyB, PublicKeyA, SharedSecretB);           // Bob computes his shared secret using Alice's public key
